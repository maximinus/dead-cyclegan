...
input_shape = (img_channels, img_height, img_width) # (3,256,256)
...
foo = ImageDataset(root, transforms_=transforms_, unaligned=True)


dataloader = DataLoader(ImageDataset(root, transforms_=transforms_, unaligned=True),
                        batch_size=1,
                        shuffle=True,
                        num_workers=n_cpu)

val_dataloader = DataLoader(ImageDataset(root, transforms_=transforms_, unaligned=True, mode='test'),
                            batch_size=5,
                            shuffle=True,
                            num_workers=n_cpu)
...
def sample_images():
    ...
    G_AB.eval()
    G_BA.eval()
    real_a = imgs['A'].type(torch.cuda.FloatTensor) # A : monet
    fake_b = G_AB(real_a).detach()
    real_b = imgs['B'].type(torch.cuda.FloatTensor) # B : photo
