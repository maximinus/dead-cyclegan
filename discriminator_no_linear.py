...
class AudioData(Dataset):
    def __init__(self, files, low_bitrate):
        super().__init__()
...
def get_audio_loaders(low_bitrate=False):
    ...
    train_data = AudioData(files[test_length:], low_bitrate)
    test_data = AudioData(files[:test_length], low_bitrate)
...
def train(model, train_dl, valid_dl):
    ...
    for epoch in range(EPOCHS):
        ...
        for x_batch, y_batch in tqdm(train_dl):
            ...
            pred = torch.flatten(model(x_batch)[:, 0])
...
def build_model():
    #show_model()
    train_data, test_data = get_audio_loaders(low_bitrate=True)
