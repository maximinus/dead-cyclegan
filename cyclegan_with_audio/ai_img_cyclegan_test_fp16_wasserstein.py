...
# 1: Use a linear activation in the discriminator (not signoid)
# 2: Use wasserstein loss
# 3: Constrain model weights to -0.01 -> 0.01
# 4: Update the critic model more often (maybe 5?)
# 5: Use RMSProp gradient descent with a small learning rate (0.00005) and no momentumn

IMAGE_FOLDER = '/home/sparky/data/MLData/CelebA/images'
ATTR_FILE = '/home/sparky/data/MLData/CelebA/attributes.txt'
IMAGE_OUTPUT = '/home/sparky/code/dead-cyclegan/output/image_results'
...
def get_faces(files):
    ...
    total_files = len(males) * 2
    print(f'* Using {total_files} files')
...
class ImageIterator:
    def __init__(self, cached=CACHED):
    ...
    def fill_cache(self):
        ...
        # 3 channels, 128x18, 2 bytes per value
        ram_usage = (len(self.male_faces) + len(self.female_faces)) * (3 * 128 * 128 * 2)
        ram_usage = ram_usage // (1024 * 1024)
        print(f'* Cached RAM usage: {ram_usage}Mb')
...
def training_loop():
    ...
    opt_gen = optim.RMSprop(list(gen_female.parameters()) + list(gen_male.parameters()),
                            lr=LEARNING_RATE)
    ...
    for epoch in range(EPOCHS):
        ...
        for male_face, female_face in tqdm(faces_iterator):
            ...
            with torch.autocast(device_type=device, dtype=torch.float16):
                ...
                wasserstein_female_loss_real = disc_female_real_loss
                wasserstein_female_loss_fake = disc_female_fake_loss * -1.0

                wasserstein_female = wasserstein_female_loss_real.mean() + wasserstein_female_loss_fake.mean()
                ...
                wasserstein_male_loss_real = disc_male_real_loss
                wasserstein_male_loss_fake = disc_male_fake_loss * -1.0
                wasserstein_male = wasserstein_male_loss_fake.mean() + wasserstein_male_loss_real.mean()

                # total loss over both discriminators
                # DISC_SLOWDOWN is a hyperfactor to slow down the learning
                d_loss = (wasserstein_female + wasserstein_male) / DISC_SLOWDOWN
...
def save_images(tensors, folder_name):
    ...
    # images come in as fp16, we need to remodel to fp32
    tensors = [x.type(torch.float32) for x in tensors]
